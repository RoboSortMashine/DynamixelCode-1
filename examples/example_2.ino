...
void setup() {
  ...
  long double pos[2] = {0, 0}; //начальные позиции сервоприводов
...
void loop() { 
  ...
  if(D_TIMER.isReady() && step == 1) {
    long double target[2] = {serv->position[serv->id_s[0]], 70}; //если не хотим менять позицию одного сервопривода, то передаём его существующие координаты.
    serv->rotate(target, 3, 300);
    D_TIMER.setInterval(3000);
    step++;
  }
  if(D_TIMER.isReady() && step == 2) {
    long double target[2] = {50, serv->position[serv->id_s[1]]}; //если не хотим менять позицию одного сервопривода, то передаём его существующие координаты.
    D_TIMER.setInterval(3000);
    serv->rotate(target, 3, 300);
    step++;
    Stop_Program = true;
